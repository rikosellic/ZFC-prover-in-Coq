...
Proof.
  ...
  assert [[ ZF;; ∀ n, n ∈ N -> n∈X -> n ∪ {n} ∈ X |-- ∀ n, n ∈ N -> n ∈ X ->  n ∪ {n} ∈ X]] by Tauto.
...
Proof.
  ...
  assert ([[ZF;; is_natural_number N |-- ∀ w, is_inductive w -> N ⊆ w]]) by Tauto.
  universal instantiation H0 [[X∩N]].
  assert ([[ZF;; is_natural_number N;; is_inductive X ∩ N |-- N ⊆ X ∩ N]]) by Tauto.
  universal instantiation H2 n.
  pose proof Intersection_iff.
  universal instantiation H4 X N n.
  assert ([[ZF;; is_natural_number N;; ∅ ∈ X;; ∀ n, n ∈ N -> n ∈ X -> n ∪ {n} ∈ X |-- n ∈ N -> n ∈ X ]]) by Tauto.
